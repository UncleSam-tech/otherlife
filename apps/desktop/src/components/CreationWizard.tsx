...
import { ArrowRight, Dices, ArrowLeft } from 'lucide-react';

export interface NewLifeWizardConfig {
...
interface CreationWizardProps {
  onBeginLife: (config: NewLifeWizardConfig) => void;
  onCancel: () => void;
}
...
export const CreationWizard: React.FC<CreationWizardProps> = ({ onBeginLife, onCancel }) => {
  ...
  const handleStart = () => {
    const config: NewLifeWizardConfig = {
  ...
  return (
    <div className="w-full max-w-2xl bg-[#0e1118] border border-amber-500/30 rounded-3xl p-8 shadow-2xl space-y-6 text-slate-100 font-sans animate-fadeIn">
      ...
      <div className="flex items-center justify-between border-b border-slate-800/80 pb-5">
        <div>
          <span className="text-xs uppercase tracking-widest text-amber-400 font-mono">Character Genesis</span>
          <h2 className="text-2xl font-serif font-bold text-slate-100 mt-0.5">Who will you be?</h2>
        </div>

        <button
          type="button"
          onClick={handleRandomize}
          className="flex items-center gap-2 bg-slate-900 hover:bg-amber-950/40 border border-slate-800 hover:border-amber-500/40 text-amber-300 px-4 py-2 rounded-xl text-xs font-serif transition-all"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-xs font-serif text-slate-400 mb-1.5">First Name</label>
          <input
            type="text"
            value={firstName}
            onChange={(e) => setFirstName(e.target.value)}
            className="w-full bg-slate-900 border border-slate-800 focus:border-amber-500/60 rounded-xl px-3.5 py-2.5 text-sm text-slate-100 font-serif focus:outline-none"
          />
        </div>

        <div>
          <label className="block text-xs font-serif text-slate-400 mb-1.5">Last Name / Family</label>
          <input
            type="text"
            value={lastName}
        ...
        <div>
          <label className="block text-xs font-serif text-slate-400 mb-1.5">Sex</label>
          <div className="flex gap-2">
            {(['Male', 'Female'] as const).map((s) => (
              <button
                ...
                type="button"
                onClick={() => setSex(s)}
                className={`flex-1 py-2.5 rounded-xl text-xs font-serif transition-all ${
                  sex === s
      ...
      <div className="space-y-3">
        <label className="block text-xs font-serif text-slate-400">Place of Birth</label>
        ...
        <div className="grid grid-cols-3 gap-3">
          {COUNTRIES.map((c, idx) => (
            ...
            >
              ...
              <div className="text-xs text-slate-500 font-mono mt-0.5">Currency: {c.currency}</div>
            </button>
          ))}
        </div>
      ...
      {/* Household Reality & Background */}
      <div className="space-y-3">
        <label className="block text-xs font-serif text-slate-400">Household Circumstances</label>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {HOUSEHOLD_TIERS.map((tier) => (
            ...
            >
              ...
              <div className="text-[11px] text-slate-400 font-sans mt-1 leading-snug">{tier.desc}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-800/80">
        ...
        <button
          type="button"
          onClick={handleStart}
          className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-serif font-bold px-6 py-3 rounded-2xl text-sm shadow-xl shadow-amber-500/20 transition-all hover:scale-105"
        >
