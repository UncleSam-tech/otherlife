...
fn test_birth_to_youth_vertical_slice() {
    ...
    assert_eq!(engine.npcs.len(), 2);

    // Scene generation
    let scene = engine.generate_today_scene();
    assert!(scene.narrative.contains("parents") || scene.narrative.contains("crib") || scene.narrative.contains("family") || scene.narrative.contains("room"));
    ...
    assert!(engine.events_ledger.len() >= 4);
    let biography = engine.get_biography();
    assert!(biography.contains("Israel") || biography.contains("Birth"));
}
